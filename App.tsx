...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCount = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/validation/count`);
        const data = await res.json();
        setSignupCount(data.count);
      } catch (err) {
        console.warn("Failed to fetch waitlist count from DB");
      }
    };
    fetchCount();
  }, []);

  // 🟢 点击报名按钮逻辑（写入数据库 + 获取真实人数 + Gemini）
  const handleJoinWaitlist = async (email: string, locState: LocationState) => {
    setIsProcessing(true);

    try {
      // 1) 写入 MongoDB
      await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/validation`, {
        method: "POST",
